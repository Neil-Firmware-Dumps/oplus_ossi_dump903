...
function configure_swappiness()
{
	...
	if [ $MemTotal -le 3145728 ]; then
		echo "vm_swappiness=180" > /proc/${oplus_path}/swappiness_para
		echo "direct_swappiness=0" > /proc/${oplus_path}/swappiness_para
	else
		echo "vm_swappiness=160" > /proc/${oplus_path}/swappiness_para
	fi
}
...
function nandswap_init(){
	...
	if [ "$set_dio" == "success" ]; then
		if [ $hybridswap_has_insmod -eq 1 ]; then
			...
			local MemTotal=${MemTotalStr:16:8}
			if [ $MemTotal -le 3145728 ]; then
				echo "200 100 200 512" > /dev/memcg/memory.avail_buffers
			elif [ $MemTotal -le 4194304 ]; then
				echo "1200 1000 1200 716" > /dev/memcg/memory.avail_buffers
			elif [ $MemTotal -le 6291456 ]; then
				echo "2000 1600 2000 1536" > /dev/memcg/memory.avail_buffers
			elif [ $MemTotal -le 8388608 ]; then
				echo "2200 1800 2200 1536" > /dev/memcg/memory.avail_buffers
			else
				echo "2000 1600 2000 1536" > /dev/memcg/memory.avail_buffers
			fi
